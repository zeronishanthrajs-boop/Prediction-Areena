    ...
    <header className="sticky top-0 z-40 w-full bg-[#07090e]/85 backdrop-blur-xl border-b border-white/[0.08]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-cyan-500 via-blue-600 to-purple-600 p-[1.5px] shadow-lg shadow-cyan-500/20 group-hover:scale-105 transition-transform">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] flex items-center justify-center text-slate-400 hover:text-slate-200 transition-colors"
            title={isMuted ? 'Unmute sound effects' : 'Mute sound effects'}
          >
            {isMuted ? <VolumeX className="w-4 h-4 text-slate-500" /> : <Volume2 className="w-4 h-4 text-cyan-400" />}
          </button>

          {/* Practice Coin Balance Chip */}
          {user && wallet && (
            <div className="flex items-center gap-2 bg-[#131926]/90 border border-amber-500/30 rounded-xl px-2.5 sm:px-3 py-1.5 shadow-md shadow-amber-500/5">
          ...
          {user && (
            <button
              ...
              className="relative flex items-center gap-1.5 bg-gradient-to-r from-purple-500/20 to-pink-500/20 hover:from-purple-500/30 hover:to-pink-500/30 border border-purple-500/40 text-purple-300 px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all shadow-sm shadow-purple-500/10"
          ...
          {user ? (
            ...
            <button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-extrabold text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-xl shadow-lg shadow-cyan-500/25 transition-all transform active:scale-95"
            >
          ...
          <button
            ...
            className="md:hidden w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-slate-400"
          >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#0a0e17] border-b border-white/10 px-4 py-3 space-y-2">
          {navLinks.map((link) => {
            const isActive = pathname === link.href;
            return (
              <Link
          ...
          {user && (
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-rose-400 hover:bg-rose-500/10 rounded-xl font-medium"
            >
